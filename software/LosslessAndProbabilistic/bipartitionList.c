...
{  
  ...
  printf("Looping over partial gene trees\n");
  //loop over partial gene trees
  for(i = 0; i < numberOfTrees; i++)
    {     
      printf("\r %d / %d (%.2f %%)",i + 1,numberOfTrees, 100.0 * (i + 1.0) / ( numberOfTrees ));
      int 
	numberOfSplits;
 ...
 //Set up list of partial bipartitions (entries in hashtable) NOTE should be done when adding entries to the hashtable
      printf("Setting up list of partial bipartitions from hashtable.\n");
 for(i = 0, entryCountPartial = 0; (unsigned int)i < genesHash->tableSize; i++)	     
    {    
      printf("\r %d / %d (%.2f %%)",i,genesHash->tableSize - 1, 100.0 * i / (genesHash->tableSize -1));
      if(genesHash->table[i] != NULL)
	{
     ...
     printf("Adjusting score for reference bipartitions\n"); 
	...
	for(i = 0; i < tr->mxtips - 3; i++)
	{	     
	  printf("\r %d / %d (%.2f %%) ",i  + 1,tr->mxtips - 3 , 100.0 * (i +1.0) / (tr->mxtips -3));
    ...
    printf("Sorting bipartitions.\n");
  qsort(partialList, entryCountPartial, sizeof(entry *), sortEntryByMask);
  
  printf("Adjusting support\n");
  	for(i = 0; (unsigned int)i + coveredBips < entryCountPartial; i++)
	{
	  printf("\r %d / %d (%.2f %%)",i - skipped ,entryCountPartial - coveredBips - newlyCoveredBips -1, 100.0 *( i -skipped) / (entryCountPartial - coveredBips - newlyCoveredBips -1));
	  e = partialList[i];
	...
	printf("\n");
      printf("Sorting by adjusted Support\n");
