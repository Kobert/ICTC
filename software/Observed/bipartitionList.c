...
void calcBipartitions_IC_Global(tree *tr, analdef *adef, char *bestTreeFileName, char *bootStrapFileName)
{
  ...
  if(allTreesEqualSize(tr, numberOfTrees, treeFile))
  ...
  else
    {
      ...
      //exit(-1); TODO delete this and the check for parsimony seed!
...
static double computeTempFreq(int id, tree * tr, unsigned int * referenceBipartition, entry ** conflictingBips, unsigned int conflicts, hashtable * genesHash, taxonMaskArray * taxonMasks, unsigned int vLength)
{
  double
        refSupport = 0;
  unsigned int 
...
// Now calculate temporary adjusted frequency (by distributing the frequency of sub bipartitions to those passed to this function.) NOTE: We would not strictly need to iterate through the same lists again if we store a map of which bipartition is covered by which bipartition.
...
static double computeObservedTempFreq( tree * tr, unsigned int * referenceBipartition, entry ** conflictingBips, unsigned int conflicts, hashtable * genesHash, taxonMaskArray * taxonMasks, unsigned int vLength)
{
  double
        refSupport = 0;
  unsigned int 
...
// Calculate temporary adjusted frequency (by distributing the frequency of sub bipartitions to those passed to this function.) NOTE: We would not strictly need to calculate the same values again if we store which bipartition is computed already.
    ...
    refBip = referenceBipartition;
	  ...
	  {	
	    ...
	    if(f != (entry*)NULL)
	    {
	      refSupport += (double)f->supportFromTreeset[0] / f->coveredNumber;  
#ifdef _DEBUG_PARTIAL_IC
	      printf("Ref:   %f / %d\n",(double)f->supportFromTreeset[0],f->coveredNumber);
#endif
	      //  printf("Ref:   %d\n",f->coveredNumber);
	    }
	  }
...
//	printf("supports: %f ",refSupport);
	  ...
	  {	
	    ...
	    if(f != (entry*)NULL)
	    {
	      assert( f->coveredNumber > 0 );
	      
	     e->tempSupport += (double)f->supportFromTreeset[0] / f->coveredNumber;
#ifdef _DEBUG_PARTIAL_IC
	     printf("Bip:   %f / %d\n",(double)f->supportFromTreeset[0],f->coveredNumber);
#endif
	     // printf("Bip:   %d\n",f->coveredNumber);
...
{  
 ...
 //Set up list of partial bipartitions (entries in hashtable)
	...
	{	      
	  ...
	  {	
	    ...
	    if(f != (entry*)NULL)
	    {
	      ...
	      f->coveredNumber++; //NOTE only needed for observed distribution
	      f->covered=TRUE;
	...
	{	      
	  ...
	  {	
	    ...
	    if(f != (entry*)NULL)
	    {
	      ...
	      if(f->coveredNumber>0); //NOTE only needed for observed distribution
	      referenceFreq[i]+=(double)f->supportFromTreeset[0]/f->coveredNumber;
	    }
	  }
	...
	for(i = 0; (unsigned int)i + coveredBips < entryCountPartial; i++)
	{
	  ...
	  {	
	    ...
	    if(f != (entry*)NULL)
	    {
	     if(f->coveredNumber>0){//NOTE only needed for observed distribution.
	      //adjust support
	      e->adjustedSupport += (double)f->supportFromTreeset[0]/f->coveredNumber;
	    }
	    }
	  } 
...
/*    
    // loop through all reference partitions to obtain ic/ica scores for probabilistic adjustment score
    for(i = 0; i < tr->mxtips - 3; i++)
    {
      ...
      tempReferenceFreq = computeObservedTempFreq(tr, refBip, conflictingBips, conflicts, genesHash, taxonMasks, vLength);
      ...
      //the following calls the lossless distribution scheme. That is, the frequency is distributed to conflicting bips only. This function cannot be called before distribution by observed frequency scheme as it overwrites needed values.
      ...
      icaNoLoss[i] = computeICAFromTemp_ValueDouble(tempReferenceFreq, conflictingBips, conflicts, FALSE);
...
qsort(partialList, entryCountPartial  - coveredBips , sizeof(entry *), sortEntryByTempSupport);

    // loop through all reference partitions to obtain ic/ica scores for observed adjustment score
    for(i = 0; i < tr->mxtips - 3; i++)
    {
      // for each reference bip, find conflicting bipartitions and store index in "conflictingBips".	      
      conflicts = 0;
      ...
      //qsort(conflictingBips, conflicts, sizeof(entry *), sortEntryByTempSupport);
#ifdef _DEBUG_PARTIAL_IC
      printf("refFrequ: %f, Bip Frequ: %f\n",tempReferenceFreq,conflictingBips[0]->tempSupport);
#endif
      icNoLoss[i]  = computeIC_ValueDouble(tempReferenceFreq, conflictingBips[0]->tempSupport , FALSE);
#ifdef _DEBUG_PARTIAL_IC
      printf("ic: %f\n",icNoLoss[i]);
...
//for(il=0 ; il < entryCountPartial - coveredBips +1; il++)
   for(il = 0 ; il < 30; il++)
  {
    printf("%3d: %2d, %2d, %f, %f",il, partialList[il]->amountTips, partialList[il]->supportFromTreeset[0], partialList[il]->adjustedSupport, partialList[il]->tempSupport);
    if(!partialList[il]->covered)
      printf(" FALSE");
    else
      printf(" TRUE");
...
rax_free(conflictingBips);
    ...
    //Do not print information about probabilistic support.
    /*
    ...
    */
    ...
    printBothOpen("Tree certainty under observed bipartition adjustment for this tree: %f\n", tcNoLoss);
    printBothOpen("Relative tree certainty under observed bipartition adjustment for this tree: %f\n\n", tcNoLoss / (double)(tr->mxtips - 3));
    
    printBothOpen("Tree certainty including all conflicting bipartitions (TCA) under observed bipartition adjustment for this tree: %f\n", tcaNoLoss);
    printBothOpen("Relative tree certainty including all conflicting bipartitions (TCA) under observed bipartition adjustment for this tree: %f\n\n", tcaNoLoss / (double)(tr->mxtips - 3));
  ...
  */
  ...
  for(i = 0; i < (unsigned int)(tr->mxtips - 3); i++)    
    printf("& %3d ", (int)(icNoLoss[i]*100)); 
     
    printf("\n");
  printf("observed ica \t");
  
  for(i = 0; i < (unsigned int)(tr->mxtips - 3); i++)     
    printf("& %3d ", (int)(icaNoLoss[i]*100));      
  ...
  for(i = 0; i < (unsigned int)(tr->mxtips - 3); i++)    
    printf("& %.4f ", (icNoLoss[i])); 
     
    printf("\n");
  printf("observed ica \t");
  
  for(i = 0; i < (unsigned int)(tr->mxtips - 3); i++)     
    printf("& %.4f ", (icaNoLoss[i]));      
