...
static void makeFileNames(void)
{
  ...
  strcat(icFileNameBranchLabelsUniform,     "RAxML_Corrected_Observed_IC_Score_BranchLabels.");
  ...
  if(processID == 0)
	...
	case CALC_BIPARTITIONS_IC:
	  ...
	  if(tr->corrected_IC_Score)
	    {
	     // printBothOpen("Tree with probabilistic corrected (for partial gene trees) IC scores as branch labels written to file:  %s\n\n", icFileNameBranchLabelsStochastic);
	      printBothOpen("Tree with observed corrected (for partial gene trees) IC scores as branch labels written to file:  %s\n\n", icFileNameBranchLabelsUniform);
